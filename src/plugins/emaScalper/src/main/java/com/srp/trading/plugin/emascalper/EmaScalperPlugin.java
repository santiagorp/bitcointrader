 ...
 * 
 */
public class EmaScalperPlugin extends BasePluginDB {
	private static Logger logger = Logger.getLogger(EmaScalperPlugin.class.getName());
	private Settings settings = null;
	private String selectedSite = null;
	private ExecutorService scalperExecutorService = null;
	private ExecutorService marketDataExecutorService = null;
	private Future<?> scalperFuture = null;
	private Future<?> marketDataFuture = null;
	private ExchangeFactory exchangeFactory;
	private Runnable scalperRunnable = null;
	private Appender appender = null;
	private String siteName;
	private String menuPosition = "main";
	Properties parametersDescription = null;

	public EmaScalperPlugin() {
		super("EmaScalperPlugin", new Version("0.9"), "EMA Scalping plugin");
	}
	...
	public List<CommandDefinition> getCommandDefinitions() {
		...
		if (menuPosition == "main") {
			if (selectedSite == null || selectedSite.isEmpty()) {
				...
				if (scalperFuture != null && !scalperFuture.isCancelled() && !scalperFuture.isDone()) {
	...
	private void forceStop() {
		if (marketDataFuture != null) {
			...
			try {
				logger.info("Finishing market data thread");
				marketDataExecutorService.awaitTermination(5, TimeUnit.SECONDS);
			} catch (InterruptedException e) {
				logger.info("Error finishing market data thread.");
				logger.info(e);
			}
			marketDataExecutorService = null;
			marketDataFuture = null;
		}
		
		if (scalperFuture != null) {
			scalperFuture.cancel(true);
			try {
				logger.info("Finishing plugin thread");
				scalperExecutorService.awaitTermination(5, TimeUnit.SECONDS);
			} catch (InterruptedException e) {
				logger.info("Error finishing plugin thread.");
				logger.info(e);
			}
			scalperExecutorService = null;
			scalperFuture = null;
		}
	}
	...
	public BasicExecutionResult executeCommand(Command cmd) {
		...
		case "Start scalping":
			...
			for (IPluginAPI plugin : server.getLoadedPlugins().values()) {
				if (plugin instanceof EmaScalperPlugin == false)
					continue;

				EmaScalperPlugin scalper = (EmaScalperPlugin) plugin;
				if (scalper.selectedSite != null && scalper.selectedSite.equals(this.selectedSite) && scalper.scalperFuture != null) {
					canScalp = false;
					break;
				}
			}
			...
			if (!canScalp) {
				...
				// Start market data extractor runnable
				marketDataExecutorService = Executors.newSingleThreadExecutor();
				marketDataFuture = marketDataExecutorService.submit(settings.getMarketData());
				
				// Start scalper runnable
				scalperExecutorService = Executors.newSingleThreadExecutor();
				scalperRunnable = getScalperRunnable();
				scalperFuture = scalperExecutorService.submit(scalperRunnable);
	...
	/**
	 * Retrieve the appropiate scalperRunnable depending of the streamming support
	 * 
	 * @return
	 */
	private Runnable getScalperRunnable() {
